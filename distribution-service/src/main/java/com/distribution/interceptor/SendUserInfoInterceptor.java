package com.distribution.interceptor;
...
public class SendUserInfoInterceptor implements HandlerInterceptor {
	private final static Logger logger = Logger
			.getLogger(SendUserInfoInterceptor.class);
	...
	public boolean preHandle(HttpServletRequest request,
		...
		if (null != pm) {
			logger.info("数据校验失败，参数【" + pm + "】不能为空");
			PrintWriter pw = response.getWriter();
			pw.write("{\"ret_code\":\"9998\",\"ret_msg\":\"参数【" + pm
					+ "】不能为空\"}");
			pw.flush();
			pw.close();
			return false;
		} else {
			// logger.info("手机号" + request.getParameter("mobile") + "数据校验成功");
			return true;
		}
	}
